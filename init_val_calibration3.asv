...
LE = 2;
rho_grid    = linspace(0.1, 0.8, LE);
a_grid      = linspace(0.2, 0.6,LE);
b_grid      = linspace(0.2, 0.7,LE);
c_grid      = linspace(0.2, 0.8,LE);
...
parameters = combvec(rho_grid, a_grid, b_grid, c_grid, muH_grid, muL_grid, sigmaH_grid, sigmaL_grid )';
n = LE^8;
results = ones(n, 9);
...
for it = 1:n
    ...
    % Before
    p.I = p.Ib;
    xb = fsolve(@excess_LD3,x0, options, p);
    wHb = xb(1);
    wLb = xb(2);
    wIb = xb(3);
    [LSHb, LD_HHb, LD_LHb, LD_IHb, LSLb, LD_HLb, LD_LLb, LD_ILb] = labor_demand3(wHb, wLb, wIb, p);
    share_entrepreneur_H_b = (highAllocation -(LSHb))/(highAllocation);
    share_entrepreneur_L_b = (lowAllocation -  (LSLb))/(lowAllocation);

    % After
    p.I = p.Ia;
    xa = fsolve(@excess_LD3,x0, options, p);
    wHa = xa(1);
    wLa = xa(2);
    wIa = xa(3);
    [LSHa, LD_HHa, LD_LHa, LD_IHa, LSLa, LD_HLa, LD_LLa, LD_ILa] = labor_demand3(wHa, wLa, wIa, p);
...
writematrix(bottom, 'initial3.txt')
